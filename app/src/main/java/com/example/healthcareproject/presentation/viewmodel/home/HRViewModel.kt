...
class HRViewModel @Inject constructor(
    ...
    fun getHeartRateDataByTimeFrame(timeFrame: String): Flow<List<Measurement>> {
        return getMeasurementRealTimeUseCase()
            .debounce(1000) // Increased to 1s to reduce rapid updates
            .map { measurements ->
                ...
                if (measurements.isEmpty()) {
                    ...
                    val filtered = measurements.filter { measurement ->
                        when (timeFrame) {
                            "MINUTE" -> measurement.dateTime.isAfter(now.minusMinutes(60)) // Extended to 60 minutes
                            "HOUR" -> measurement.dateTime.isAfter(now.minusHours(6)) // Extended to 6 hours
                ...
                Timber.e(e, "Error in getHeartRateDataByTimeFrame")
                emit(emptyList())
            }
    }
